...
##############
#load shapefile
shp <- shapefile("./Export_Output.shp")

#choose predictors
predNames <- c("bio3","bio7","bio8","bio9","bio15","bio17","bio18")

presClim <-stack(list.files(r"(E:\Help\六十三\correct)", 
                            pattern = ".tif$", full.names = TRUE, 
                            ignore.case = TRUE)[8:14])
...
futClims <-stack(list.files(r"(E:\Help\六十三\correct)", pattern = ".tif$", full.names = TRUE, ignore.case = TRUE)[1:7])
...
##############
cl <- makeCluster(14) #ideally should be run in parallel to reduce computing time
